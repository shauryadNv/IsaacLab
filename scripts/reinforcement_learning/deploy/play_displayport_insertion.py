...
class LeappDisplayportPolicy:
    ...
    def __call__(self, obs=None) -> torch.Tensor:
        ...
        rnn_vals = self._read_feedback_packed()
        self.last_rnn_in = None if rnn_vals is None else rnn_vals.copy()
...
class InferenceLogger:
    ...
    * ``target_joint_pos_{i}`` — sim-only: ``joint_pos_target`` as it stands after
    ...
    def print_step(
        ...
        if rnn_in is not None:
            rnn_vals = np.asarray(rnn_in, dtype=np.float64).reshape(-1)
            print(f"[rnn_in] dim={rnn_vals.size} ||rnn_in||={np.linalg.norm(rnn_vals):.4f}")
        if rnn_out is not None:
