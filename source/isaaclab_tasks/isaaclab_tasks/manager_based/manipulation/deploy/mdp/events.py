...
class reset_plug_at_goal_curriculum(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        # Depth ranges [min, max] (m) along the insertion axis, measured from the
        # socket keypoint origin (0 == fully seated). `at_goal_depth_range` seeds the
        # near-goal (shallow / partially inserted) curriculum envs; if None it falls
        # back to [0, insertion_length]. `approach_depth_range` seeds the harder
        # approach envs further out along the axis; if None the legacy default-plug-
        # pose approach placement is used instead. Setting `approach_depth_range`
        # closes the reset "deadzone" between the at-goal band and the approach band.
    ...
    def __call__(
        ...
        if self.approach_depth_range is None:
            # ---- Legacy placement (kept for callers that don't set depth ranges) ----
            ...
            # ---- Unified along-axis placement (deadzone fix) ----
            # Both at-goal and approach envs are placed along the insertion axis at a
            # sampled depth from the socket keypoint origin, at the socket-relative
            # goal orientation. At-goal envs use the shallow `at_goal_depth_range`;
            # approach envs use the deeper `approach_depth_range` plus lateral
            # (perpendicular) randomization from `normal_pose_range`. Sampling both
            # bands from the same origin removes the reset "deadzone" gap that opens
            # up when the at-goal band and the approach band don't overlap.
            at_goal_mask = torch.rand(num_envs, device=env.device) < current_at_goal_prob
            ...
            # Lateral (perpendicular) randomization from normal_pose_range; approach
            # envs only (at-goal envs stay exactly on the insertion axis).
            pose_range = self.normal_pose_range
            rand_pos = torch.zeros(num_envs, 3, device=env.device)
            for i, key in enumerate(["x", "y", "z"]):
                rng = pose_range.get(key, [0.0, 0.0])
