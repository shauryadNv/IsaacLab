...
"""Class-based termination terms for gear assembly and cable insertion manipulation environments."""
...
class reset_when_plug_dropped(ManagerTermBase):
    ...
    def __init__(self, cfg: TerminationTermCfg, env: ManagerBasedEnv):
        ...
        if "end_effector_body_name" not in cfg.params:
            raise ValueError("'end_effector_body_name' is required in reset_when_plug_dropped configuration.")
        if "grasp_rot_offset" not in cfg.params:
            raise ValueError("'grasp_rot_offset' is required in reset_when_plug_dropped configuration.")
        if "grasp_offset" not in cfg.params:
            raise ValueError("'grasp_offset' is required in reset_when_plug_dropped configuration.")
...
class reset_when_plug_orientation_exceeded(ManagerTermBase):
    ...
    Computes the relative orientation between the plug (with grasp rotation offset
    applied) and the end effector. If the roll, pitch, or yaw angles exceed the
    configured thresholds, the environment is reset.
    """
    ...
    def __init__(self, cfg: TerminationTermCfg, env: ManagerBasedEnv):
        ...
        if "end_effector_body_name" not in cfg.params:
            raise ValueError(
                "'end_effector_body_name' is required in reset_when_plug_orientation_exceeded configuration."
            )
        if "grasp_rot_offset" not in cfg.params:
            raise ValueError(
                "'grasp_rot_offset' is required in reset_when_plug_orientation_exceeded configuration."
            )
