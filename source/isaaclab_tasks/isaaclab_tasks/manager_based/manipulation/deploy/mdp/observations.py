...
class gear_shaft_pos_w(ManagerTermBase):
    ...
    def __call__(
        ...
        base_pos = _tensor_data_to_torch(asset.data.root_pos_w)
        base_quat = _tensor_data_to_torch(asset.data.root_quat_w)
...
class gear_shaft_quat_w(ManagerTermBase):
    ...
    def __call__(
        ...
        real_env = _leapp_real_env(env)
        base_quat = _tensor_data_to_torch(real_env.scene[self.asset_cfg.name].data.root_quat_w)
        if _is_leapp_export_env(env):
            from leapp import annotate
            from leapp.utils.tensor_description import TensorSemantics
