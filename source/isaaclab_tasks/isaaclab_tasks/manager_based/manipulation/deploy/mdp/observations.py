...
class rigid_object_pos_w(ManagerTermBase):
    """Rigid object position in world frame with optional offset.

    Generic observation term that returns the position of any RigidObject asset
    in the environment frame. An optional 3D offset can be applied in the object's
    local frame before returning the position.

    Args:
        asset_cfg: The asset configuration. Required.
        offset: A 3D offset list [x, y, z] applied in the object's local frame.
            Defaults to [0, 0, 0].
...
class rigid_object_quat_w(ManagerTermBase):
    """Rigid object orientation in world frame.

    Generic observation term that returns the orientation of any RigidObject asset.
    The quaternion is canonicalized so that the w component is positive.

    Args:
        asset_cfg: The asset configuration. Required.

    Returns:
        Object orientation tensor as a quaternion with shape (num_envs, 4).
    """
