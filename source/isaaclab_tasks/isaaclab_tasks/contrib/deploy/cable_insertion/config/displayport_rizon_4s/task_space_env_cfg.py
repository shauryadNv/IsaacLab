...
at-goal curriculum, mirroring the joint-space ``joint_pos_env_cfg.py``.
"""
...
# OSC gains: d_gain = 2 * sqrt(p_gain) * damping_ratio
...
# Gripper TCP offset from the flange body in the flange's local frame [m].
# The policy observes the TCP pose (where the plug is held), not the raw flange.
...
# experiment commit sets these switches to a specific combination. Mirrors the
# joint-space EXP block, minus sysid: the joint-space sysid (identified joint-PD
# gains + delayed joint action) is invalid under OSC because the arm joint PD is
# zeroed for operational-space control.
# --- EXP TOGGLES START ---
...
EXP_OBS_NOISE_M = 0.005   # socket-position observation noise [m] applied at inference (0.0=none, 0.005=5mm)
# --- EXP TOGGLES END ---
...
class Rizon4sTaskSpaceDisplayportInsertionEnvCfg(DisplayportInsertionEnvCfg):
    ...
    def __post_init__(self):
        ...
        # OSC requires arm joints to be pure torque-controlled. Zero the arm PD gains
        # (damping is provided by OSC via ``motion_damping_ratio_task``).
