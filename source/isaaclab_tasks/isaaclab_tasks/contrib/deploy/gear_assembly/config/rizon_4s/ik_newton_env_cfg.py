...
class Rizon4sGearAssemblyIKNewtonEnvCfg(joint_pos_env_cfg.Rizon4sGearAssemblyEnvCfg):
    ...
    directly. Task-space control is generally more sample-efficient for contact-rich insertion than
    joint-space control. The gripper remains a fixed mimic mechanism (not commanded by the policy).

    Note:
        Newton IK consumes the replicated robot prototype, so this variant requires a Newton preset
        (``presets=default`` or ``presets=newton_hydroelastic``); it is not compatible with
        ``presets=physx``.
    """
