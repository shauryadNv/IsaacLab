...
class NewtonManager(PhysicsManager):
    ...
    def _solver_accepts_update_contacts_state(solver: SolverBase) -> bool:
        """Return whether ``solver.update_contacts`` accepts a state argument."""
        try:
    ...
    def _prepare_cuda_graph_capture(cls) -> None:
        """Prepare solver resources after CUDA graph capture begins.

        The default implementation is a no-op. Solvers with stream events or
        other capture-owned resources can override this hook.
        """
